...
public class ArrivalsListFragment extends ListFragment {
    ...
    private ArrayList<Arrival> getArrivalsForSelectedStop() {
        ...
        ArrayList<Arrival> sixArrival = new ArrayList<>();

        if (selected != null){

            for (Arrival next: selected){
                sixArrival.add(next);
            }

//            Iterator<Arrival> arrivalIterator = selected.iterator();
//            while (arrivalIterator.hasNext()){
//                sixArrival.add(arrivalIterator.next());
//            }
        }
